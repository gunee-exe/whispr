...
class ConfirmationCard extends ConsumerWidget {
  ...
  Widget _buildReady(BuildContext context, WidgetRef ref) {
    ...
    final dateFmt = dueAt != null
        ? DateFormat('EEE, MMM d • h:mm a').format(dueAt)
        : null;
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          data.taskTitle ?? 'Untitled reminder',
        ...
        Row(
          children: [
            ElevatedButton(
              onPressed: () => onConfirm(data),
              child: const Text('Got it'),
            ),
            const SizedBox(width: 12),
            TextButton(onPressed: onDismiss, child: const Text('Edit')),
          ],
        ),
      ],
    );
  ...
  Widget _buildClarification(BuildContext context, WidgetRef ref) {
    return Column(
      ...
      children: [
        Text(
          data.question ?? 'Can you clarify?',
          style: WhisprText.body(size: 17, weight: FontWeight.w600),
        ),
        if (data.quickReplyOptions != null &&
            data.quickReplyOptions!.isNotEmpty) ...[
              ...
              return ActionChip(
                ...
                onPressed: () {
                  ...
                  ref
                      ...
                      .submitText(opt, clarificationContext: data.partialParse);
                },
              );
            }).toList(),
          ),
    ...
    return Column(
      ...
      children: [
        ...
        if (data.interpretationSingleTask != null)
          OutlinedButton(
            onPressed: () => onConfirm(data.interpretationSingleTask!),
            child: Text(
              'One task: "${data.interpretationSingleTask!.taskTitle}"',
            ),
          ),
        const SizedBox(height: 8),
        if (data.interpretationTwoTasks != null)
          ElevatedButton(
            ...
            child: Text(
              'Two tasks: ${data.interpretationTwoTasks!.map((t) => t.taskTitle).join(" + ")}',
            ),
          ),
      ],
    );
