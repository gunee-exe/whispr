...
// The Flutter-generated @UIApplicationMain AppDelegate is preserved below
...
@objc class AppDelegate: FlutterAppDelegate {
    ...
    private var activeActivities: [String: Any] = [:] // [activityId: Activity<WhisprActivityAttributes>]
    ...
    private func handleStartSingle(_ args: Any?, result: FlutterResult) {
        ...
        do {
            if #available(iOS 16.2, *) {
                ...
                // Store reference keyed by triggerId for later updates/ends.
                // Swift generics prevent storing Activity<T> directly in [String:Any]
                // without type erasure — use the activity's id string instead.
                result(activity.id)
            }
        } catch {
    ...
    private func handleEnd(_ args: Any?, result: FlutterResult) {
        ...
        if #available(iOS 16.2, *) {
            Task {
                // End the activity matching this triggerId.
    ...
    private func handleEndAll(result: FlutterResult) {
        if #available(iOS 16.2, *) {
            Task {
                for activity in Activity<WhisprActivityAttributes>.activities {
                    await activity.end(dismissalPolicy: .immediate)
