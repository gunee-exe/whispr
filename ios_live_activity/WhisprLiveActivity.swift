...
//   1. File → New → Target → Widget Extension → name it "WhisprWidget".
//   2. Add "NSSupportsLiveActivities" = YES to the extension's Info.plist.
//   3. Add ActivityKit framework to the extension target.
//   4. Ensure the App Group entitlement matches between Runner and WhisprWidget
//      (com.whispr.app.shared) so the platform channel bridge can share data.
//   5. Run on a real device — Live Activities do not render in the simulator
//      until Xcode 15+ / iOS 17+ simulator builds.
...
/// Animated countdown clock label (updated by the OS using Date-based content).
@available(iOSApplicationExtension 16.2, *)
...
/// SwiftUI countdown ring that mirrors the Dart CountdownRing widget.
@available(iOSApplicationExtension 16.2, *)
struct CountdownRingView: View {
    ...
    var body: some View {
        ZStack {
            ...
            // ActivityKit Date-based timer arc — animates without server pushes.
            Circle()
    ...
    private var progressFraction: Double {
        // For SwiftUI previews / non-animating contexts only.
        // In a real Live Activity the OS handles the Date-based animation.
